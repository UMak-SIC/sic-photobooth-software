...
-- 1. Create template_placements table
...
-- 2. Create template_overlays table
...
-- 3. Data Migration: migrate legacy templates.placements JSONB rows into template_placements
DO $$
DECLARE
  t_row RECORD;
  p_elem JSONB;
    ...
    FOR t_row IN SELECT id, placements FROM templates WHERE jsonb_typeof(placements) = 'array' AND jsonb_array_length(placements) > 0 LOOP
      FOR p_elem IN SELECT * FROM jsonb_array_elements(t_row.placements) LOOP
        INSERT INTO template_placements (
          ...
          COALESCE((p_elem->>'x')::INT, 0),
          COALESCE((p_elem->>'y')::INT, 0),
          COALESCE((p_elem->>'width')::INT, 100),
          COALESCE((p_elem->>'height')::INT, 100),
          COALESCE((p_elem->>'rotation')::INT, 0),
          COALESCE((p_elem->>'borderRadius')::INT, 0),
...
-- 4. Seed Canonical Default Templates and Placements if none exist
DO $$
DECLARE
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM templates WHERE id = classic_id) THEN
    INSERT INTO templates (
      ...
      background_path, is_active, required_capture_count, countdown_seconds
    ) VALUES (
      classic_id,
