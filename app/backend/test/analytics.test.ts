...
describe('Analytics & KPI Engine', () => {
  ...
  describe('GET /api/admin/analytics Endpoint', () => {
    it('returns structured analytics data with summary, templates, layout breakdown, and comparison', async () => {
      // Create mock event with unique name to prevent collision across test runs
      const eventName = `Analytics Test Event ${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
      ...
      try {
        ...
        await dbRepository.selectTemplate(s1.id, {
        ...
        await dbRepository.selectTemplate(s2.id, {
        ...
        await dbRepository.setSessionFrame(s3.id, 'frame-flipbook-arcade', {
        ...
        await dbRepository.deleteEvent(event.id);
      }
    });
  });
});
